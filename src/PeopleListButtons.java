 ...
 *				   if (!clickedButtonsArray[BtnNum - ADJ]) {
 *					   button1.setText(button1.getText() + "\n ATTENDANCE ADDED");
 *					   clickedButtonsArray[BtnNum - ADJ] = true;
...
public class PeopleListButtons extends JPanel{
	...
	private final static int NUM_BUTTONS = 26;
	...
	public PeopleListButtons() {
		...
		button21 = new JButton("Kevin Tee");
		...
		button26 = new JButton("Bridget L");
		button26.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				if (!clickedButtonsArray[26 - ADJ]) {
					button26.setText(button26.getText() + "\n ATTENDANCE ADDED");
					clickedButtonsArray[26 - ADJ] = true;
					clickHistory.add(button26);
				}
			}	
		});
	...
	public void saveButton(String excelFilePath, String[] names, int[] attendances) throws IOException {  
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
    		...
    		excelWriter.writeExcel(listPerson, excelFilePath + file.getName(), workbook);
    		
    		JOptionPane.showMessageDialog(null, "File " + file.getName() + " saved sucessfully.");
        } 
		else {
