...
public class BibleReadingLogApplication{
	...
	private String windowsExcelFilePath = "\\Users\\andre\\Dropbox\\JavaProgram\\BibleReadingLogApplication\\";
	private static String os = System.getProperty("os.name");
	
	private boolean isWindows = this.getIsWindows();
	...
	public BibleReadingLogApplication() {
		...
		saveButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					if (isWindows) {
						((PeopleListButtons) buttonPanel).saveButton(windowsExcelFilePath, names, attendances);
					} else {
	...
	private boolean getIsWindows() {
		return os.substring(0,1).equalsIgnoreCase("w");
	}
